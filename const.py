"""SQLite database for PeakSense with device management."""
...
class Storage:
    """Manages PeakSense database with events, devices, and signatures."""
    ...
    def _init_db(self):
        ...
        # Signatures table (learning data)
        c.execute("""
        ...
        # Detections table (recognition results)
        c.execute("""
    ...
    def update_event_device(self, event_id, device_id, confidence=1.0):
        """Update event with detected device."""
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute(
    ...
    def label_event(self, event_id, label):
        """Label an event (old method for compatibility)."""
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
    ...
    def delete_device(self, device_id):
        """Delete a device and its signatures."""
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
    ...
    def record_signature(self, device_id, event):
        """Record a spike signature for a device."""
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        
        # Check if similar signature exists
        c.execute(
            """SELECT id, count FROM signatures 
               WHERE device_id = ? AND ABS(peak - ?) < 50
               ORDER BY ABS(peak - ?) LIMIT 1""",
            (device_id, event['peak'], event['peak'])
        )
        existing = c.fetchone()
        
        if existing:
            # Increment count for similar signature
            sig_id, count = existing
            c.execute(
                "UPDATE signatures SET count = count + 1 WHERE id = ?",
                (sig_id,)
            )
        else:
    ...
    def get_device_signatures(self, device_id):
        """Get all signatures for a device."""
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute(
            "SELECT * FROM signatures WHERE device_id = ? ORDER BY count DESC",
            (device_id,)
        )
        rows = [dict(row) for row in c.fetchall()]
        conn.close()
    ...
    def delete_signature(self, signature_id):
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("DELETE FROM signatures WHERE id = ?", (signature_id,))
        conn.commit()
        conn.close()
    ...
    def record_detection(self, event_id, device_id, confidence):
        """Record an automatic device detection."""
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("""
    ...
    def get_detection(self, event_id):
        """Get detection result for an event."""
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
    ...
    def update_detection_feedback(self, detection_id, is_correct):
        """Update detection with user feedback (for model improvement)."""
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute(
            "UPDATE detections SET is_correct = ? WHERE id = ?",
            (is_correct, detection_id)
        )
        conn.commit()
        conn.close()
    ...
    def get_device_statistics(self, device_id):
        ...
        c.execute(
            """SELECT AVG(peak) as avg_peak, MIN(peak) as min_peak, 
                      MAX(peak) as max_peak, AVG(duration) as avg_duration
    ...
    def get_accuracy_stats(self):
        ...
        c.execute(
            "SELECT COUNT(*) FROM detections WHERE is_correct IS NOT NULL"
        )
        total = c.fetchone()[0]
